    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">문의 관리</h1>
          <p className="text-gray-600 mb-2">회원들의 문의를 확인하고 답변하세요.</p>
          <p className="text-xs text-gray-400 mb-8">미답변 문의에 답변을 달면 해당 유저에게 알림이 전송됩니다.</p>
            ...
            {([["all","전체"], ["pending","미답변"], ["answered","답변완료"]] as const).map(([val, label]) => (
              <button key={val} onClick={() => setAdminFilter(val)}
                className={`px-4 py-2 rounded-full text-sm font-medium transition ${
                  adminFilter === val ? "bg-blue-600 text-white" : "bg-white text-gray-700 border border-gray-300 hover:border-blue-400"
                }`}>{label} {val === "pending" ? `(${allInquiries.filter(i => i.status === "pending").length})` : ""}</button>
            ))}
          </div>

          {loading ? <div className="text-center py-12 text-gray-400">로딩 중...</div> : filteredInquiries.length === 0 ? (
            <div className="text-center py-12 text-gray-400">문의가 없습니다.</div>
          ) : (
            <div className="space-y-4">
              {filteredInquiries.map((inq) => (
                <div key={inq.id} className="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
                  <button onClick={() => setExpandedId(expandedId === inq.id ? null : inq.id)}
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-gray-900 text-sm">{inq.title}</span>
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        ...
                        <span className="text-gray-400 text-sm">{expandedId === inq.id ? "▲" : "▼"}</span>
                      </div>
                    </div>
                  </button>
                  ...
                  {expandedId === inq.id && (
                    <div className="px-6 pb-6 border-t border-gray-100">
                      ...
                      <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                        <p className="text-xs text-gray-500 mb-2 font-medium">문의 내용</p>
                        <p className="text-gray-800 text-sm whitespace-pre-wrap break-words">{inq.content}</p>
                      </div>
                      ...
                      {inq.reply && (
                        <div className="mt-3 p-4 bg-blue-50 rounded-lg border border-blue-100">
                          ...
                          <p className="text-gray-800 text-sm whitespace-pre-wrap break-words">{inq.reply}</p>
                        </div>
                      )}

                      {/* 답변 입력 */}
                      <div className="mt-4">
                        <p className="text-xs font-medium text-gray-700 mb-2">{inq.reply ? "답변 수정" : "답변 작성"}</p>
                        <textarea
                          value={replyText[inq.id] ?? (inq.reply || "")}
                          onChange={(e) => setReplyText((prev) => ({ ...prev, [inq.id]: e.target.value }))}
                          rows={3} placeholder="답변을 입력하세요"
                          className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <p className="text-gray-600 mb-2">궁금한 점이나 불편한 점을 운영진에게 직접 전달해보세요.</p>
        <p className="text-xs text-gray-400 mb-8">문의를 등록하면 운영진이 검토 후 답변을 남깁니다. 본인의 문의와 답변은 이 페이지에서 확인할 수 있습니다.</p>

        {!userId ? (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 text-center">
            <p className="text-blue-800 mb-2">문의하려면 로그인이 필요합니다.</p>
          ...
          <>
            ...
            {showForm && (
              <div className="bg-white rounded-xl shadow-md p-8 mb-8 border border-gray-100">
                <h2 className="text-xl font-bold text-gray-900 mb-6">문의 작성</h2>
        ...
        {userId && (
          ...
          myInquiries.length === 0 ? (
            <div className="text-center py-12 text-gray-400">등록한 문의가 없습니다.</div>
          ) : (
                  ...
                  {expandedId === inq.id && (
                    <div className="px-6 pb-5 border-t border-gray-100">
                      ...
                      {inq.reply ? (
                        ...
                        <p className="text-xs text-gray-400 mt-4">아직 답변이 등록되지 않았습니다. 조금만 기다려주세요.</p>
                      )}
                      <button onClick={() => handleDeleteInquiry(inq.id)}
                        className="mt-3 text-xs text-gray-400 hover:text-red-500 transition">문의 삭제</button>
                    </div>
                  )}
                </div>
              ))}
