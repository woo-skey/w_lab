...
export default function BarDetailPage() {
  ...
  if (!bar) return <div className="tone min-h-screen flex items-center justify-center text-white/40">Bar를 찾을 수 없습니다.</div>;
  ...
  return (
    <div className="tone min-h-screen">
      <div className="tone-wrap max-w-3xl mx-auto px-4 py-8 md:py-12">
        ...
        {/* Bar 정보 */}
        <div className="glass-card card rounded-2xl p-6 mb-6">
          <div className="flex justify-between items-start mb-4">
            <h1 className="section-title text-3xl font-bold text-white">{bar.bar_name}</h1>
        ...
        {/* 즐겨찾기 멤버 */}
        {favorites.length > 0 && (
          <div className="glass-card card rounded-2xl p-5 mb-6">
            <h2 className="meta text-sm font-semibold text-white/60 mb-3">❤️ {favorites.length}명이 즐겨찾기 중</h2>
            <div className="flex flex-wrap gap-2">
              {favorites.map((f, i) => (
                <span key={i} className="chip px-2 py-0.5 bg-white/5 rounded-full text-xs text-white/50 border border-white/10">
        ...
        {/* 코멘트 */}
        <div className="glass-card card rounded-2xl p-5">
          <h2 className="font-bold text-white mb-4">방문 후기</h2>

          {comments.length === 0 ? (
            <p className="empty text-white/30 text-sm mb-4 px-3 py-2">아직 방문 후기가 없습니다.</p>
          ) : (
          ...
          {userId ? (
            <form onSubmit={handleAddComment} className="flex gap-2">
              ...
              <button type="submit" disabled={submitting || !commentText.trim()}
                className="cta px-4 py-2 bg-indigo-500/80 text-white text-sm rounded-lg hover:bg-indigo-500 disabled:opacity-40 transition">
                등록
              </button>
            </form>
          ) : (
