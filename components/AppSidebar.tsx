  ...
  if (isMobile) {
    return (
      <div className="relative min-h-screen" style={{ background: bg, fontFamily: SF }}>
        ...
        {showDrawer && (
          <div className="fixed inset-0 z-30" style={{ background: "rgba(0,0,0,0.45)" }} onClick={() => setShowDrawer(false)} />
        )}
          ...
          <div className="fixed inset-0 z-50 flex flex-col" style={{ background: T.notifBg, fontFamily: SF }}>
            <div className="flex items-center justify-between px-4 py-4 flex-shrink-0" style={{ borderBottom: `1px solid ${T.border}` }}>
              ...
              <div className="flex items-center gap-3">
                {unreadCount > 0 && (
                  <button onClick={handleMarkAllRead} className="text-sm" style={{ color: T.textMuted }}>
                    모두 읽음
                  </button>
                )}
                ...
                <button onClick={() => setShowMobileNotif(false)} className="text-lg px-1" style={{ color: T.textSecondary }}>
                  ✕
                </button>
              </div>
            </div>
                ...
                notifications.map((n) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm leading-snug" style={{ color: n.is_read ? T.notifMuted : T.notifText }}>
                        {n.message}
                      </p>
    ...
    <div className="flex min-h-screen" style={{ background: bg, fontFamily: SF }}>
      ...
      <aside className="fixed top-0 left-0 h-screen w-56 z-20 flex flex-col" style={sidebarStyle}>
        <div className="px-5 pt-8 pb-4">
          <Link href="/">
            <h1 className="font-bold text-base tracking-tight cursor-pointer transition" style={{ color: T.textPrimary }}>
              🥃 위스키 연구소
            </h1>
          </Link>
        </div>
        ...
        <div className="px-3 mb-4">
          <button
            ...
            style={{ border: `1px solid ${T.searchBorder}`, background: T.searchBg, color: T.textSecondary }}
          >
            <span className="flex items-center gap-2 text-sm">
              <span style={{ color: T.textMuted }}>🔎</span>
              <span>검색 열기</span>
            </span>
            <span className="text-[10px] px-1.5 py-0.5 rounded border border-white/15" style={{ color: T.textMuted }}>
              Ctrl+K
            </span>
          </button>
        </div>
        ...
        <div className="px-3 mb-2">
          <p className="text-xs px-2 mb-1 uppercase tracking-wider" style={{ color: T.textMuted }}>
            메뉴
          </p>
        </div>
          ...
          {userId ? (
            <>
              <div className="relative" ref={notifRef}>
                ...
                {showNotifications && (
                  <div
                    className="absolute bottom-full left-0 mb-2 w-72 rounded-xl overflow-hidden z-50 flex flex-col"
                    style={{
                  ...
                  >
                    <div className="px-4 py-3 flex justify-between items-center flex-shrink-0" style={{ borderBottom: `1px solid ${T.border}` }}>
                      ...
                      <div className="flex items-center gap-2">
                        {unreadCount > 0 && (
                          <button onClick={handleMarkAllRead} className="text-xs hover:text-indigo-400 transition" style={{ color: T.textMuted }}>
                            모두 읽음
                          </button>
                        )}
                        {notifications.length > 0 && (
                    ...
                    <div className="overflow-y-auto flex-1">
                      {notifications.length === 0 ? (
                        <p className="text-center text-sm py-8" style={{ color: T.textMuted }}>
                          알림이 없습니다
                        </p>
                      ) : (
                        notifications.map((n) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="leading-snug" style={{ color: n.is_read ? T.notifMuted : T.notifText }}>
                                {n.message}
                              </p>
                              <p className="text-xs mt-1" style={{ color: T.textMuted }}>
              ...
              <Link href="/mypage" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors hover:bg-black/5" style={{ color: T.textSecondary }}>
                <span className="w-6 h-6 rounded-full bg-indigo-500/50 flex items-center justify-center text-xs text-white font-bold flex-shrink-0">
                  {userName.slice(0, 1).toUpperCase()}
                </span>
                <span className="truncate">{userName}</span>
            ...
            <Link href="/login" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors hover:bg-black/5" style={{ color: T.textSecondary }}>
              <span className="text-base w-5 text-center">🔐</span>
              <span>로그인</span>
            </Link>
          )}
      ...
      <div className="flex-1 ml-56 relative z-10 min-h-screen">{children}</div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none select-none z-0">
        <div className="absolute -top-40 -left-40 w-[600px] h-[600px] rounded-full opacity-20" style={{ background: o1, animation: "orb-float-1 18s ease-in-out infinite" }} />
        <div className="absolute top-1/3 -right-60 w-[500px] h-[500px] rounded-full opacity-15" style={{ background: o2, animation: "orb-float-2 22s ease-in-out infinite" }} />
        <div className="absolute -bottom-40 left-1/3 w-[400px] h-[400px] rounded-full opacity-10" style={{ background: o3, animation: "orb-float-3 15s ease-in-out infinite" }} />
      </div>
    </>
  );
}
